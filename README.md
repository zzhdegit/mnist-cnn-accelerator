...
以下数据来自本地 Vivado 2024.2 / xsim 最新验证结果，标签为 `iter13_conv2_64oc2col`。
...
以下数据来自 `iter13_conv2_64oc2col` routed utilization report，器件为 `xc7z020clg400-1`。
...
以下数据来自 `iter13_conv2_64oc2col` routed timing summary。
...
```powershell
powershell -ExecutionPolicy Bypass -File fpga\scripts\run_accuracy_sim.ps1 -Tag iter13_conv2_64oc2col_100 -NImages 100
```

重新生成 Vivado synthesis / implementation 报告：

```powershell
& "D:\Xilinx\Vivado\2024.2\bin\vivado.bat" -mode batch -source fpga\scripts\run_vivado_reports.tcl -tclargs iter13_conv2_64oc2col
```
...
- 当前 README 只保留最新 `iter13_conv2_64oc2col` 版本内容。
